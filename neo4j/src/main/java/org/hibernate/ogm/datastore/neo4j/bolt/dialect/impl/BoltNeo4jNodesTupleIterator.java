...
public class BoltNeo4jNodesTupleIterator implements ClosableIterator<Tuple> {
	...
	private Tuple createTuple(NodeWithEmbeddedNodes node) {
		Map<String, Node> toOneEntities = BoltNeo4jDialectOperations.findToOneEntities( tx, node, entityKeyMetadata, tupleContext, entityQueries );
		return new Tuple(
				new BoltNeo4jTupleSnapshot( node, entityKeyMetadata, toOneEntities, tupleContext ) );
	}
