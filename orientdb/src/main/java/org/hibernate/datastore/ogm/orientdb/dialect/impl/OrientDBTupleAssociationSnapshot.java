    ...
    private Map<String, Object> collectProperties() {
		...
		for ( String associationColumn : associatedEntityKeyMetadata.getAssociationKeyColumns() ) {
			String targetColumnName = associatedEntityKeyMetadata.getCorrespondingEntityKeyColumn( associationColumn );
                        if ( targetNode.containsField( targetColumnName ) ) {
					properties.put( associationColumn, targetNode.getOriginalValue( targetColumnName ) );
				}
                }  */      
		...
		/* for ( int i = 0; i < associationKey.getColumnNames().length; i++ ) {
			if ( ownerNode.containsField( associationKey.getEntityKey().getColumnNames()[i] ) ) {
				properties.put( associationKey.getColumnNames()[i], ownerNode.getOriginalValue(associationKey.getEntityKey().getColumnNames()[i] ) );
			}
		} */
